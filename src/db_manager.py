...
class DBManager:
    ...
    def get_companies_and_vacancies_count(self):
        ...
        with self.conn.cursor() as cur:
            cur.execute("""SELECT company_name as company_name, SUM(open_vacancies) as open_vacancies FROM employers GROUP BY id_emp""")
            df = pd.DataFrame(cur.fetchall(), columns=[description[0] for description in cur.description])
            df = df.rename(columns={'company_name': 'Имя компании', 'open_vacancies': 'Количество вакансий'})
            cur.close()
    ...
    def get_all_vacancies(self):
        ...
        with self.conn.cursor() as cur:
            cur.execute("""SELECT vacancy_name, company_name, COALESCE(salary,0) as salary, vacancy_url
    ...
    def get_vacancies_with_keyword(self, search_text):
        ...
        with self.conn.cursor() as cur:
            cur.execute("""SELECT DISTINCT vacancy_name FROM vacancies 
